...
if not durations or any(d == 0 for d in durations):
    print(f"ℹ️  No valid durations provided. Using default of {default_duration} ms for all {num_images} frames.")
    durations = [default_duration] * num_images
elif len(durations) != num_images:
    raise ValueError(f"⚠️  {num_images} images found, but {len(durations)} durations provided.")
